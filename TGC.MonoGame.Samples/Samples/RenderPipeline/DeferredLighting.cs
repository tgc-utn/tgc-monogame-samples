...
namespace TGC.MonoGame.Samples.Samples.RenderPipeline
{
    public class DeferredLighting : TGCSample
    {
        ...
        private Vector3 ambientLightPosition = new Vector3(180, 180, 50);
        private Vector3 ambientLightDiffuseColor = new Vector3(.6f, .6f, .6f);
        private Vector3 ambientLightSpecularColor = new Vector3(.6f, .6f, .6f);
        private Vector3 lightCursorPosition = new Vector3(-345f, -20f, 80f);
        private Vector3 lightCursorColor = Color.White.ToVector3();
        private float pointRadius = 50f;
        private List<PointLight> pointLights = new List<PointLight>();
        private Model _sphere;
        
        public override void Initialize()
        {
            _camera = new FreeCamera(GraphicsDevice.Viewport.AspectRatio, new Vector3(-300f, 150f, 300f));
        ...
        protected override void LoadContent()
        {
            ...
            ModifierController.AddVector("Ambient position", OnAmbientPositionChange, ambientLightPosition);
            ModifierController.AddColor("Ambient color", OnAmbientColorChange, new Color(ambientLightDiffuseColor));
        ...
        void DrawAmbient()
        {
            ...
            _effectDeferred.Parameters["lightPosition"].SetValue(ambientLightPosition);
            _effectDeferred.Parameters["lightDiffuseColor"].SetValue(ambientLightDiffuseColor);
            _effectDeferred.Parameters["lightSpecularColor"].SetValue(ambientLightSpecularColor);
        ...
        void DrawCursor()
        {
            ...
            _effectModel.Parameters["color"].SetValue(lightCursorColor);
            AssignEffectToModel(_effectModel, _sphere);
            foreach (var mesh in _sphere.Meshes)
            {
                var w = Matrix.CreateTranslation(lightCursorPosition);
        ...
        private void OnPointRemoveClick()
        {
            if(pointLights.Count > 0)
                pointLights.RemoveAt(pointLights.Count - 1);
            
        }
        private void OnPointPositionChange(Vector3 position)
        {
            lightCursorPosition = position;
        }
        private void OnPointColorChange(Color color)
        {
            lightCursorColor = color.ToVector3();
        }
        ...
        {
            pointLights.Add(new PointLight(lightCursorPosition, lightCursorColor, lightCursorColor, pointRadius));
        }
        private void OnAmbientColorChange(Color color)
        {
            ambientLightDiffuseColor = color.ToVector3();
            ambientLightSpecularColor = ambientLightDiffuseColor;
        }

        private void OnAmbientPositionChange(Vector3 pos)
        {
            ambientLightPosition = pos;
        }
        ...
        private void GeneratePointLights()
        {
            ...
            r = 40f;
            pointLights.Add(new PointLight(pos, col, col, r));
